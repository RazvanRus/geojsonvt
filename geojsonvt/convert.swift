...
class Convert {
    ...
    class func convert(data: JSON, tolerance: Double) -> [ProjectedFeature] {
        ...
        if ((data["type"] as! String) == "FeatureCollection") {
            ...
            Convert.convertFeature(features: &features, feature: ["geometry": data as AnyObject], tolerance: tolerance)
        }

        return features
    }

    class func convertFeature( features: inout [ProjectedFeature], feature: JSON, tolerance: Double) {
        ...
        if (type == "Point") {
            ...
            let point = Convert.projectPoint(p: LonLat(coordinates: coordinates))
            ...
            print("Unsupported GeoJSON type: \(geometryType)")
    ...
    class func project(lonlats: [LonLat], tolerance: Double = 0) -> ProjectedGeometryContainer {
        ...
        for i in 0..<lonlats.count {
            projected.members.append(Convert.projectPoint(p: lonlats[i]))
        }
        if (tolerance > 0) {
    ...
    class func projectPoint(p: LonLat) -> ProjectedPoint {
        ...
        let sine = sin(p.lat * .pi / 180)
        let x = p.lon / 360 + 0.5
        let y = 0.5 - 0.25 * log((1 + sine) / (1 - sine)) / .pi

        return ProjectedPoint(x: x, y: y, z: 0)
    }

    class func calcSize( geometryContainer: inout ProjectedGeometryContainer) {
    ...
    class func calcBBox( feature: inout ProjectedFeature) {
    ...
    class func calcRingBBox( minPoint: inout ProjectedPoint, maxPoint: inout ProjectedPoint, geometry: ProjectedGeometryContainer) {
